...
#    Y=[]
#    bsz=2000
#    Y.append([os.path.join(specific_dir,f"test_freeze_{model}_bs{bsz}_{dev}") for model in models])
#    Y.append([os.path.join(specific_dir,f"test_split_{model}_bs{bsz}_{dev}") for model in models])
#    bsz=8000
#    Y.append([os.path.join(specific_dir,f"test_freeze_{model}_bs{bsz}_{dev}") for model in models])
#    Y.append([os.path.join(specific_dir,f"test_split_{model}_bs{bsz}_{dev}") for model in models])
#    
...
#    Y.append([os.path.join(specific_dir,f"split_{model}_bs{bsz}_{dev}") for model in models])
    ...
    for y in Y:
        ...
        text.append(t)
    #sys_legends = [f"{BASELINE}, B=2000",f"{SPLIT}, B=2000",f"{BASELINE}, B=8000",f"{SPLIT}, B=8000"]
    sys_legends = [f"{BASELINE}, B=128",f"{SPLIT}, B=128"]
    #for m in models:
    #    print(m.title())
    ...
    #for y in Y:
    #    print(y)
    plot_bars(Y, sys_legends, xtick_labels, hatches, "Models", "Execution Time (sec.)", f"results/{exp_name}_{dev}_{bw}", text=text,colors=colors, rotation=40)
###################################################################################################
##EXP2: BW exp
#exp_name="bw_exp"
#model = 'alexnet'
...
#Y=[]
#Y.append([os.path.join(specific_dir,f"vanilla_{bw/1024}_{model}") for bw in BW])

    ...
    if vanilla:
        if transformed:
            subprocess.Popen(shlex.split(server + f'"cd /root/swift/swift/proxy/mllib;\
                                     ...
                                     python3 server.py --cached --vanilla --transformed > test/{dataset}/server_{ba}vanilla_{model}_bs{bsz}_bw{m_bw}_{"cpu" if CPU_ else "gpu"}_transformed" \
                                                     &'))
        else:
            subprocess.Popen(shlex.split(server + f'"cd /root/swift/swift/proxy/mllib;\
                                                                 ST_AUTH_VERSION=1.0 ST_AUTH=http://192.168.0.246:8080/auth/v1.0     ST_USER=test:tester ST_KEY=testing \
                                                                 python3 server.py --cached --vanilla > test/{dataset}/server_{ba}vanilla_{model}_bs{bsz}_bw{m_bw}_{"cpu" if CPU_ else "gpu"}" \
                                                                                 &'))
    else:
        subprocess.Popen(shlex.split(server + f'"cd /root/swift/swift/proxy/mllib;\
                                                             ST_AUTH_VERSION=1.0 ST_AUTH=http://192.168.0.246:8080/auth/v1.0     ST_USER=test:tester ST_KEY=testing \
                                                             python3 server.py --cached --transformed > test/{dataset}/server_{ba}{model}_bs{bsz}_bw{m_bw}_{"cpu" if CPU_ else "gpu"}" \
                                                                             &'))
...
def run_model_exp_ba(batch_size, model, freeze_idx, ba, m_bw, CPU_=False, dataset='imagenet', vanilla=False, transformed=True):
    ...
    if vanilla:
        if transformed:
            ...
            os.system(client + f'python3 {execfile} --dataset {dataset} --model {model} --num_epochs 1 --batch_size {batch_size}\
                                                             --freeze --freeze_idx {freeze_idx} {"--cpuonly" if CPU_ else ""} --cached --transformed {opt_adapt}\
                                                              > {logdir}/{dataset}/{ba}vanilla_{model}_bs{batch_size}_bw{m_bw}_{"cpu" if CPU_ else "gpu"}_transformed')
        else:
            os.system(client + f'python3 {execfile} --dataset {dataset} --model {model} --num_epochs 1 --batch_size {batch_size}\
                                                             --freeze --freeze_idx {freeze_idx} {"--cpuonly" if CPU_ else ""} --cached {opt_adapt}\
                                                              > {logdir}/{dataset}/{ba}vanilla_{model}_bs{batch_size}_bw{m_bw}_{"cpu" if CPU_ else "gpu"}')
    else:
        ...
        os.system(client + f'python3 {execfile} --dataset {dataset} --model my{model} --num_epochs 1 --batch_size {batch_size}\
                                             --freeze --freeze_idx {freeze_idx}  --use_intermediate {"--cpuonly" if CPU_ else ""} --cached --transformed {opt_adapt}\
                                              > {logdir}/{dataset}/{ba}{model}_bs{batch_size}_bw{m_bw}_{"cpu" if CPU_ else "gpu"}')
